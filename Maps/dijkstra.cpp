...
Dijkstra::Dijkstra(Nodo* ciudades[54])
{
...
void Dijkstra::resolverD(int nodo){
    ...
    while(nodo!=-1){
        ...
        while(temp!=NULL){
            if(ciudadesD[nodo]->visitado==false){
                ...
                if(ciudadesD[temp->nodoAdj->valor]->costo>temp->peso+ciudadesD[nodo]->costo){
                    ciudadesD[temp->nodoAdj->valor]->costo=temp->peso+ciudadesD[nodo]->costo;
...
int Dijkstra::buscar(QString ciudad){
    for(int cont=0; cont<54; cont++){
